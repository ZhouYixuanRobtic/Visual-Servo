...
ControlTeleop::ControlTeleop(bool publish_vel, double max_linear_velocity,
  ...
  bl_content_sub_ =
      nh_.subscribe("bl_content", 100, &ControlTeleop::BLCallback, this);
  if (publish_vel) {
  ...
  bl_watchdog_timer_ = nh_.createTimer(ros::Duration(WATCHDOG_PERIOD_),
                                       &ControlTeleop::BLwatchdog, this, true);
...
void ControlTeleop::JoyCallback(const sensor_msgs::JoyConstPtr &msg) {
  ...
  if (msg->axes[2] == -1 && msg->buttons[0])  // LT+A
  {
    control_trigger_ = ManipulateOn;
  } else if (msg->axes[2] == -1 && msg->buttons[1])  // LT+B
  {
    control_trigger_ = ChargeOn;
  } else if (msg->axes[2] == -1 && msg->buttons[2])  // LT+X
  {
    control_trigger_ = HomeOn;
  } else if (msg->axes[2] == -1 && msg->buttons[3])  // LT+Y
  {
    control_trigger_ = UpOn;
  } else if (msg->axes[2] == -1 && msg->axes[6] == 1)  // LT+LEFT
  {
    control_trigger_ = LinearBack;
  } else if (msg->axes[2] == -1 && msg->axes[6] == -1)  // LT+RIGHT
  {
    control_trigger_ = LinearForward;
  } else if (msg->axes[2] == -1 && msg->axes[7] == 1)  // LT+UP
  {
    control_trigger_ = LinearUp;
  } else if (msg->axes[2] == -1 && msg->axes[7] == -1)  // LT+DOWN
  {
    control_trigger_ = LinearDown;
  } else if (msg->axes[2] == -1 && msg->buttons[5])  // LT+RB
  {
    control_trigger_ = ArmEmergencyChange;
  } else if (msg->axes[2] == -1 && msg->buttons[4])  // LT+LB
  {
    control_trigger_ = CutBack;
  } else if (msg->axes[2] == -1 && msg->axes[5] == -1)  // LT+RT
  {
  ...
  else if (msg->axes[5] == -1 && msg->buttons[0])  // RT+A
  {
    control_trigger_ = KnifeOff;
  } else if (msg->axes[5] == -1 && msg->buttons[1])  // RT+B
  {
    control_trigger_ = ClockGo;
  } else if (msg->axes[5] == -1 && msg->buttons[2])  // RT+X
  {
    control_trigger_ = AntiClockGo;
  } else if (msg->axes[5] == -1 && msg->axes[6] == 1)  // RT+left
  {
    control_trigger_ = SingleAntiClockGo;
  } else if (msg->axes[5] == -1 && msg->axes[6] == -1)  // RT+right
  {
    control_trigger_ = SingleClockGo;
  } else if (msg->axes[5] == -1 && msg->buttons[3])  // RT+Y
  {
    control_trigger_ = KnifeOn;
  } else if (msg->axes[5] == -1 && msg->buttons[10])  // RT+R_STICK
  {
    control_trigger_ = ShutDown;
  } else if (msg->axes[5] == -1 && msg->buttons[4])  // RT+LB
  {
    control_trigger_ = SteeringIn;
  } else if (msg->axes[5] == -1 && msg->buttons[5])  // RT+RB
  {
  ...
  } else if (msg->buttons[4])  // LB
  {
    control_trigger_ = NavOn;
  } else if (msg->buttons[5])  // RB
  {
    control_trigger_ = NavPause;
  } else if (msg->buttons[6])  // BACK(CENTER LEFT)
  {
    control_trigger_ = MappingOff;
  } else if (msg->buttons[7])  // START(CENTER RIGHT)
  {
    control_trigger_ = MappingOn;
  } else if (msg->buttons[9])  // LEFT SITCK
  {
    control_trigger_ = RobotArmOn;
  } else if (msg->buttons[10])  // RIGHT STICK
  {
