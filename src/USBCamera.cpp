...
USBCamera::USBCamera(std::string intrinsic_path)
    : INTRINSIC_PATH_(std::move(intrinsic_path)) {
  ...
  if (isOpened) {
    ...
    resolution_ = cv::Size(capture_->get(cv::CAP_PROP_FRAME_WIDTH),
                           capture_->get(cv::CAP_PROP_FRAME_HEIGHT));
    video_thread_ptr_.reset(
        new boost::thread(boost::bind(&USBCamera::video_worker, this)));
  } else {
    std::cout << "No camera detected, is the camera connected to the computer!"
              << std::endl;
  }
}
USBCamera::~USBCamera() {
  ...
  try {
    ...
    while (true) {
      ...
      boost::this_thread::sleep(boost::posix_time::microseconds(
          (int)(1E6 / FPS_ - (endTime - startTime).total_microseconds())));
    }
...
void USBCamera::setParameter(cv::Size &resolution, int fps) {
  ...
  if (isOpened) {
    ...
    capture_->set(cv::CAP_PROP_FRAME_WIDTH, resolution_.width);
    capture_->set(cv::CAP_PROP_FRAME_HEIGHT, resolution_.height);
  }
}
...
void USBCamera::getIntrinsicParameter(double &fx, double &fy, double &cx,
                                      double &cy) {
  fx = fx_;
  fy = fy_;
  cx = cx_;
...
UndistortedGroup &USBCamera::getUndistortedGroup() {
  ...
  undistortedGroup_.intrinsic_matrix = cv::getOptimalNewCameraMatrix(
      intrinsic_matrix_, distortion_coefficients_, resolution_, 1, resolution_);
  cv::undistort(frame_, undistortedGroup_.frame, intrinsic_matrix_,
