...
void Detector::vertical_threshold(cv::Mat src) {
  cv::Mat_<Vec3b>::iterator it1{src.begin<Vec3b>()};
  while (it1 != src.end<Vec3b>()) {
    if ((*it1)[2] - (*it1)[1] > 30 && (*it1)[2] - (*it1)[0] > 30 &&
        (*it1)[2] > 100) {
      (*it1)[0] = 255;
      (*it1)[1] = 255;
      (*it1)[2] = 255;
 ...
 * @param processType   [process type, OBLIQUE for oblique knife image, VERTICAL
 * for vertical knife image] note: the private variable operate_image is
...
cv::Mat Detector::pretreatment(int processType) {
  ...
  if (colorOn_) {
    cv::GaussianBlur(this->operate_image, gaussianImg, Size(5, 5), 0, 0,
                     cv::BORDER_DEFAULT);
  } else
    gaussianImg = this->operate_image.clone();
  switch (processType) {
    case OBLIQUE:
      if (colorOn_) oblique_threshold(gaussianImg);
      break;
    case VERTICAL:
      if (colorOn_) vertical_threshold(gaussianImg);
      break;
    default:
      break;
  }
  if (colorOn_) {
    cv::cvtColor(gaussianImg, grayImg, COLOR_BGRA2GRAY);
    radon::radonTransform(grayImg, grayImgRadon, radonAngleRange,
                          radonOperation);
  } else
    radon::radonTransform(gaussianImg, grayImgRadon, radonAngleRange,
                          radonOperation);
 ...
 * @param targetPointX              [The x coordinate of point online you wanna
 * get,-1 for non-use]
 * @param targetPointY              [The y coordinate of point online you wanna
 * get,-1 for non-use]
 * @return the complete coordinate of a point online known one coordinate.
 */
cv::Point Detector::getPointOnline(const cv::Size& priorRadonImgSize,
                                   const cv::Size& subsequentRadonImgSize,
                                   const cv::Point& referencePoint,
                                   const int& targetPointX,
                                   const int& targetPointY) {
  int x0, y0;
  double k;
...
void Detector::refineLoc() {
  ...
  if (colorOn_) {
    cv::GaussianBlur(this->operate_image, gaussianImg, Size(15, 15), 0, 0,
                     cv::BORDER_DEFAULT);
    cv::cvtColor(gaussianImg, grayImg, COLOR_BGRA2GRAY);
  ...
  cv::Point initPoint(beginPoint.x - RectSize.width / 1.5,
                      beginPoint.y - RectSize.height / 1.5);
  cv::Rect rect(initPoint, RectSize);

  cv::Mat refineImg{grayImg(rect)};
  cv::Mat refineImgRadon;
  radon::radonTransform(refineImg, refineImgRadon, radonAngleRange,
                        radonOperation);

  cv::Point maxLoc;
  cv::Mat mask{Mat::zeros(refineImgRadon.size(), CV_8UC1)};
  mask(cv::Rect(cv::Point(20, 0), cv::Size(130, refineImgRadon.size().height)))
      .setTo(255);
  cv::minMaxLoc(refineImgRadon, nullptr, nullptr, nullptr, &maxLoc, mask);
  cv::Point refinedBeginPoint{getPointOnline(refineImg.size(),
                                             refineImgRadon.size(), maxLoc,
                                             (beginPoint - initPoint).x, -1)};
...
cv::Point Detector::get_BeginPoint(const cv::Mat& test_image_) {
  ...
  if (!colorOn_) {
    cv::GaussianBlur(clone_image, clone_image, Size(15, 15), 0, 0,
                     cv::BORDER_DEFAULT);
    cv::cvtColor(clone_image, clone_image, COLOR_BGRA2GRAY);
  ...
  verticalMaxLoc.x = verticalImg.size().height / 2 - verticalMaxLoc.y +
                     this->operate_image.size().width / 2;
  beginPoint = getPointOnline(this->operate_image.size(), obliqueImg.size(),
                              obliqueMaxLoc, verticalMaxLoc.x);

  refineLoc();

  this->borderPoint =
      getPointOnline(this->operate_image.size(), obliqueImg.size(),
                     obliqueMaxLoc, -1, this->operate_image.rows);
  if (this->borderPoint.x < 0 || this->borderPoint.x > this->operate_image.cols)
    this->borderPoint =
        getPointOnline(this->operate_image.size(), obliqueImg.size(),
                       obliqueMaxLoc, this->operate_image.cols, -1);

  if (traceResultOn_) {
    cv::circle(this->display_image, beginPoint, 5, Scalar(255, 0, 0), -1);
    cv::line(this->display_image, beginPoint, borderPoint, Scalar(0, 0, 255),
             2);
    cv::line(this->display_image,
             cv::Point(verticalMaxLoc.x, this->operate_image.rows),
             cv::Point(verticalMaxLoc.x, 0), Scalar(255, 255, 0), 2);
...
std::vector<cv::Point> Detector::get_knifeTrace(const cv::Mat& test_image_) {
  ...
  if (traceResultOn_) {
    cv::circle(this->display_image, beginPoint, 5, Scalar(255, 0, 0), -1);
    cv::polylines(this->display_image, knife_trace, false, Scalar(0, 255, 0),
                  2);
    imshow("detection result", this->display_image);
    waitKey(0);
    cv::destroyAllWindows();
...
std::vector<cv::Point> Detector::get_knife_trace(bool debug) {
  ...
  box.size.height = 15;
  box.angle =
      atan2((beginPoint - borderPoint).y, (beginPoint - borderPoint).x) *
      180.0 / M_PI;

  box.center.x = (borderPoint.x + beginPoint.x) / 2;
  box.center.y = (borderPoint.y + beginPoint.y) / 2;

  DrawBox(box, mask);
  cv::floodFill(mask, box.center, 255, nullptr, cvScalarAll(0), cvScalarAll(0),
                CV_FLOODFILL_FIXED_RANGE);

  cv::Mat newImg, newImgCanny;
  if (colorOn_) {
    cv::GaussianBlur(this->operate_image, newImgCanny, Size(15, 15), 0, 0,
                     cv::BORDER_DEFAULT);
    cv::cvtColor(newImgCanny, newImgCanny, COLOR_BGRA2GRAY);
  ...
  if (debug) {
    ...
    cv::Mat contoursDisplayImg =
        Mat::zeros(this->operate_image.size(), CV_8UC1);
...
std::vector<cv::Point> Detector::get_traceSegments(
    const std::vector<cv::Point>& knife_trace, int segement_number) {
  std::vector<cv::Point> segments;

  if (!knife_trace.empty()) {
    int interval = knife_trace.size() / segement_number;
    if (segement_number == 0) return knife_trace;
