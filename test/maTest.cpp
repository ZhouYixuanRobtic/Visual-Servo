...
Listener::Listener(int tag_id) : tagID_(tag_id) {
  ...
  tag_info_sub =
      nh.subscribe("TagsDetected", 10, &Listener::tagInfoCallback, this);
}
Listener::~Listener() { delete serviceCaller; }
...
double Listener::getParameter() {
  ...
  try {
    result =
        doc["ID" + std::to_string(tagID_)]["steeringDistance"].as<double>();
  } catch (YAML::InvalidScalar) {
    ROS_ERROR("tagParam.yaml is invalid.");
  }
  ROS_INFO_STREAM("the tag id is " << tagID_ << " the steeringDistance is "
                                   << result);
  return result;
}
int main(int argc, char** argv) {
  ...
  CONTROLTELEOP::ControlTeleop controlTeleop(true, max_linear_velocity,
  ...
  listener.pox_system("gnome-terminal -x bash -c \"roslaunch rubber_navigation
  baseOnly.launch publish_robot_source_odom_tf:=true\";exit;exec bash");
  */
    ...
    switch (controlTeleop.getControlTrigger()) {
      case ManipulateOn:
        ...
        if (!listener.serviceCaller->srvCalling())
          listener.serviceCaller->callSrv(
              visual_servo::manipulate::Request::CUT);
        break;
      case ChargeOn:
        if (!listener.serviceCaller->srvCalling())
          listener.serviceCaller->callSrv(
              visual_servo::manipulate::Request::CHARGE);
        break;
      case UpOn:
        if (!listener.serviceCaller->srvCalling())
          listener.serviceCaller->callSrv(
              visual_servo::manipulate::Request::UP);
        break;
      case HomeOn:
        if (!listener.serviceCaller->srvCalling())
          listener.serviceCaller->callSrv(
              visual_servo::manipulate::Request::HOME);
        break;
      case CutBack:
        if (!listener.serviceCaller->srvCalling())
          listener.serviceCaller->callSrv(
              visual_servo::manipulate::Request::CUTBACK);
        break;
      case LinearUp:
        if (!listener.serviceCaller->srvCalling())
          listener.serviceCaller->callSrv(
              visual_servo::manipulate::Request::LINEAR,
              Eigen::Vector3d(0.0, 0.0, 0.0015));
        break;
      case LinearDown:
        if (!listener.serviceCaller->srvCalling())
          listener.serviceCaller->callSrv(
      ...
      case RobotArmOn: {
        ...
        listener.pox_system(
            "gnome-terminal -x bash -c \"roslaunch visual_servo "
            "visual_servo_real.launch\";exit;exec bash;");
        break;
      }
      case NavigationOn: {
        ros::V_string AllNodes;
        ...
        sleep(10);
        listener.pox_system(
            "gnome-terminal -x bash -c \"roslaunch rubber_navigation "
            "rubber_navigation.launch\";exit;exec bash");
        break;
      }
      case ShutDown: {
        ros::V_string AllNodes;
        ros::master::getNodes(AllNodes);
        for (auto& node_name : AllNodes) {
          if (node_name != "/joystick" && node_name != "/maTest" &&
              node_name != "/rosout") {
            listener.pox_system(("rosnode kill " + node_name).c_str());
          }
        }
        sleep(1);
        listener.pox_system(
            "gnome-terminal -x bash -c \"shutdown\"; exec bash;");
      }
      case MappingOn: {
        /*
        ros::V_string AllNodes;
        ...
        listener.pox_system("gnome-terminal -x bash -c \"roslaunch
        cartographer_ros demo_velodyne2d.launch\";exit;exec bash;");
        */
        break;
      }
      case MappingOff: {
        ...
        for (auto& node_name : AllNodes) {
          if (node_name == "/rviz") {
            ...
            sleep(1);
            std::string temp{ros::package::getPath("rubber_navigation") +
                             "/config/map/map.bag.pbstream"};
            std::string final_temp =
                "gnome-terminal -x bash -c \"rosservice call /write_state "
                "\"filename: \'" +
                temp +
                "\' include_unfinished_submaps: \'true\'\"\";exit;exec bash;";
